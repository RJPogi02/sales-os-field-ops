      ...
      <div style={cardGrid}>
        ...
        <article style={{ ...card, opacity: signedIn ? 1 : .58 }}>
          ...
          {workspace?.id ? <div style={{ display: 'grid', gap: 12 }}>
            ...
            <label style={field}><span>New team name</span><input style={input} value={organizationName} onChange={(event) => setOrganizationName(event.target.value)} placeholder={`${config?.companyName || 'Company'} Sales`} /></label>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: .55 }}><span style={{ height: 1, flex: 1, background: 'currentColor' }} /><small>OR</small><span style={{ height: 1, flex: 1, background: 'currentColor' }} /></div>
